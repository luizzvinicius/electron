...
app.whenReady().then(async () => {  
  ...
  // let init = exec(
  //   `${path.join(process.env.ROOT!, "release/win-unpacked/reources/app/dist/ipfs-cluster-service/ipfs-cluster-service.exe")} init -f http://bafybeigq7gjeusrv3tu7kaiv5yirr3vb76xqysjalwlec5fwkmudqx2bh4.ipfs.localhost:8080/`,
  //   (error, stdout: string, stderr: string) => {
  //     console.log(error), console.log(stderr), console.log(stdout);
  //   }
  // );
  // let bootstrap = exec(
  //   `${path.join(process.env.ROOT!, "release/win-unpacked/reources/app/dist/ipfs-cluster-service/ipfs-cluster-service.exe")} -f daemon --bootstrap /ip4/127.0.0.1/tcp/9092/p2p/12D3KooWR8MxQgHGLF5vx1ShZy3Dkb5ZMjgm5MjVpJaDpi1Qqpwp`,
